var jsFileLocation = $('script[src*=hgoe_js]').attr('src');
jsFileLocation = jsFileLocation.replace('hgoe_js.php', '');
	...
	if ($result->num_rows > 0) {
		...
		while($row = $result->fetch_assoc()) {
			...
			if($currDate > $datum) {
				...
				echo "$.ajax({";
				echo "	url: jsFileLocation + '../script_veranstaltung_absagen.php?noAlert=1&id=" . $id . "', ";
				echo "  async: false, ";
